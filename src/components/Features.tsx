    ...
    <section className="section section-gray" id="features">
      <div className="container-xl px-10">
        <motion.div className="text-center mb-16" initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
          ...
          <h2 className="font-syne text-[46px] font-extrabold tracking-[-1.2px] leading-[1.1] mb-4">Industries <span className="text-gradient-gold">We Serve</span></h2>
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-7">
          {industries.map((item, i) => (
            <motion.div key={item.title} className="group bg-white p-8 rounded-xl border border-surface-200 hover:-translate-y-1 hover:shadow-[0_12px_40px_rgba(0,0,0,0.06)] hover:border-gold/20 transition-all duration-300"
              initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.08 }}>
              <div className="w-12 h-12 rounded-xl bg-surface-50 flex items-center justify-center mb-5 group-hover:scale-110 group-hover:bg-gold-pale transition-all">
                <item.icon size={22} className="text-muted-600 group-hover:text-gold transition-colors" />
              </div>

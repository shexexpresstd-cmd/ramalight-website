...
export default function Hero() {
  ...
  return (
    <section ref={heroRef} className="bg-brand min-h-screen flex items-center relative overflow-hidden py-[130px]">
      {/* Grid */}
      ...
      <div className="absolute inset-0 pointer-events-none">
        {[...Array(20)].map((_, i) => (
          <div key={i} className="absolute w-0.5 h-0.5 bg-accent-light rounded-full opacity-0"
      ...
      <div className="container-xl relative z-10 flex flex-col lg:flex-row items-center gap-16 lg:gap-28 px-10">
        ...
        <div className="flex-1 text-center lg:text-left" style={{ opacity: 1, transform: 'none' }}>
          <motion.div className="inline-flex items-center gap-2 border border-accent/30 text-accent-light px-5 py-1.5 rounded-full text-xs font-semibold mb-6 bg-accent-pale/5"
            initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }}>
            <span className="w-2 h-2 bg-accent-light rounded-full" style={{ animation: 'pulse 2s infinite' }} /> RAMALIGHT CO. LIMITED — Now Accepting Bulk Orders
          </motion.div>
          ...
          <motion.div className="flex flex-col sm:flex-row items-center gap-3 justify-center lg:justify-start"
            ...
            <button onClick={() => scrollTo('specs')} className="w-full sm:w-auto px-8 py-4 text-base font-semibold text-white/50 border border-white/10 rounded-lg hover:border-accent-light/40 hover:text-accent-light transition-all">
              View Specifications
            </button>
          </motion.div>
        ...
        <motion.div className="relative shrink-0"
          ...
          <div className="w-[340px] sm:w-[440px] bg-brand-50/80 backdrop-blur-sm rounded-2xl border border-white/[0.04] p-8 text-center">
            <div className="w-full h-48 bg-gradient-to-br from-accent/10 to-accent-pale/5 rounded-xl flex items-center justify-center mb-4 border border-white/[0.03]">
            ...
            <div className="grid grid-cols-4 gap-2 mt-5">
              {[['10–15%','Conc.'],['KS/KEBS','Std.'],['6 Mo.','Shelf'],['Bulk','Supply']].map(([v,k]) => (
                <div key={k} className="bg-brand/50 rounded-lg py-3 border border-white/[0.03] hover:bg-accent-pale/5 hover:scale-105 transition-all">
      ...
      <div className="absolute bottom-10 left-1/2 -translate-x-1/2 flex flex-col items-center gap-2 text-white/20 text-[11px] tracking-[2px] uppercase"
        ...
        <div className="w-px h-12 bg-gradient-to-b from-white/20 to-transparent relative overflow-hidden">
          <div className="absolute top-[-100%] w-full h-full bg-accent-light" style={{ animation: 'scrollDown 2s ease-in-out infinite' }} />
        </div>
      </div>
    </section>
  );
